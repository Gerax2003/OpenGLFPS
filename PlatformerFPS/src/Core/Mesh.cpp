...
void createVAOandVBO(unsigned int& VAO, unsigned int& VBO, std::vector<float>& buffer)
{
...
void objParser(std::string filePath, float scale, unsigned int& VAO, unsigned int& VBO, int& bufferSize)
{
	...
	//Debug::printVectors(positions, uvCoords, normals, faces);
	//Debug::printVertexBufferParse(model.buffer);
}

Mesh::Mesh(std::string filePath, float scale) : Resource(filePath, ResourceType::MESH)
{
	objParser(filePath, scale, VAO, VBO, bufferSize);
}
